...
def main():
    ...
    parser.add_argument('--gt_data_path', type=str, default='/workspace/av2format/test')
    parser.add_argument('--submit_data_path', type=str, default='./lightning_logs/version_23/test_results/')
    ...
    for _, file_name in enumerate(tqdm(file_names, desc='under assessment')):
        ...
        # ground-truth
        file_name = file_name.replace('_submission', '')
        with open(os.path.join(args.gt_data_path, file_name), 'rb') as file:
