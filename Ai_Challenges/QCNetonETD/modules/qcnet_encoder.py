...
class QCNetEncoder(nn.Module):
    ...
    def prepare_map_encoder_inputs(self, data:HeteroData):
        ...
        # [5]map_type 추가 
        map_type = data['map_polygon']['type'].long().contiguous()                # M

        if self.dataset == 'ETRI_Dataset':
            if self.input_dim == 2:
                magnitude = data['map_point']['magnitude']
                
                # 1. 원본 magnitude
                magnitude_raw = magnitude.unsqueeze(-1)
                
                # 2. 정규화된 magnitude (배치별 정규화)
                magnitude_normalized = (magnitude / (magnitude.max() + 1e-6)).unsqueeze(-1)
                
                # Enhanced point features (N x 2) - 로그 스케일 제거
                x_pt = torch.cat([magnitude_raw, magnitude_normalized], dim=-1)
                x_pl = None
            else:
        ...
        #[6]map_type을 넘김. 
        return (pos_pt, orient_pt, pos_pl, orient_pl, orient_vector_pl, x_pt, x_pl, map_polygon_batch, map_point_batch, edge_index_pt2pl, edge_index_pl2pl, map_type)
    ...
    def prepare_agent_encoder_inputs(self, data: HeteroData, map_enc: Mapping[str, torch.Tensor], isFake=False):
        ...
        # Add magnitude information for agent encoder (raw data only)
        if self.dataset == 'ETRI_Dataset' and 'magnitude' in data['map_point']:
            magnitude_info = data['map_point']['magnitude']  # N (raw point magnitudes)
        else:
            magnitude_info = None
        ...
        return (map_enc_x_pl, mask, pos_a, motion_vector_a, head_a, pos_pl, orient_pl, vel, agent_type, batch_s, batch_pl, magnitude_info)
