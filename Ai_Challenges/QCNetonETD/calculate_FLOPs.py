...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', type=str, default="/workspace/Ai_Challenges/ETRITrajPredChallenage/competition_data/") #root는 변환된 QCNet형식 데이터들이 저장된 최상위 폴더를 가리킴
    ...
    parser.add_argument('--devices', type=int, default=4, help='The number of possible GPU devices')
    ...
    parser.add_argument('--output_head', action='store_true') # default는 False -> 실험 ㄱㄱ
    ...
    parser.add_argument('--dropout', type=float, default=0.3062)
    parser.add_argument('--lr', type=float, default=0.000023689) #원본 = 5e-4
    parser.add_argument('--weight_decay', type=float, default=0.00046187)
    ...
    # Save FLOPs result to flops.txt file
    # import os
    # flops_file_path = os.path.abspath('flops_hangawi_trial_0.txt')
    # with open(flops_file_path, 'w') as f:
    #     f.write(f"{GFLOPs:.2f}\n")
    
    # print(f"FLOPs result saved to: {flops_file_path}")
    # print(f"FLOPs value: {GFLOPs:.2f} G")