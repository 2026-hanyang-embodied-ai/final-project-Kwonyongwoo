...
class VonMisesNLLLoss(nn.Module):
    ...
    def forward(self,
        ...
        conc = conc.clone()
        with torch.no_grad(): conc.clamp_(min=self.eps)
    ...
    # Ex) loc=mean_angle= π/4 (45도) / con=concentration=집중도=2.0 (높을수록 확신 up) / target = π/3

    #     -2.0 * cos(π/3-π/4) +log(2*math.pi) +_log_modified_bessel_fn(2.0, *order*=0)

    #     *= -1.932 + 1.838 + 0.688 ≈ 0.594*
