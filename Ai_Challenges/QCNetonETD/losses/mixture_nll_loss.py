...
class MixtureNLLLoss(nn.Module):
    ...
    def forward(self,
                pred: torch.Tensor,     #여러 모드의 예측
                target: torch.Tensor,   #여러 모드의 실제값
                prob: torch.Tensor,    #각 모드의 확률
                mask: torch.Tensor,    #각 모드의 마스크
                ptr: Optional[torch.Tensor] = None,
                joint: bool = False) -> torch.Tensor:
        if isinstance(self.nll_loss, nn.ModuleList):
            nll = torch.cat(
                [self.nll_loss[i](pred=pred[..., [i, target.size(-1) + i]],target=target[..., [i]].unsqueeze(1))
                 for i in range(target.size(-1))], #각 차원별로 (x, y, heading)
                dim=-1)
        else:
        ...
        #핵심 : 각 모드의 확률과 NLL을 결합 
        log_pi = F.log_softmax(prob, dim=-1) #각 모드의 log확률 
